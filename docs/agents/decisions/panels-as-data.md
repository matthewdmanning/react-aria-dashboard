...
- This is a one-off template product — each user re-runs the card-authoring process themselves, not a one-time build. Variant-authoring (a genuinely new visual shape) happens weekly early, tapering off; arrangement changes weekly-to-monthly; data changes daily. The recurring pain point is variant-authoring, not data or arrangement.
...
- Two data shapes cover every real card need: mechanical data (a declarative mapping from a source, e.g. Google Calendar) and judgment data (an agent or human writes schema-matching data directly — a prioritized task list — no formatter beyond identity).

Given that, cards became config entries referencing a fixed, code-defined variant registry; formatters became identity, a named built-in, or a declarative spec; the entire code-authoring path — draft pipeline, on-disk card package, regex/AST governance — was deleted rather than fixed. Variant-authoring (the one place code still changes) moved to ordinary source edits, governed by normal code review; lint tooling for it was explicitly deferred as unnecessary for a one-off project.
